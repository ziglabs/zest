...
test "lengths are equal" {
    ...
    try expect(status_enum_length == Status.status_codes_u16.len);
    try expect(status_enum_length == Status.status_codes_string.len);
}

test "array values are equal" {
    for (Status.status_codes_u16) |value, i| {
        try expect(value == try std.fmt.parseUnsigned(u16, Status.status_codes_string[i], 10));
    }
}

test "invalid values return an error" {
    const expected_error = StatusError.InvalidStatusCode;
    try expectError(expected_error, Status.fromString(""));
    try expectError(expected_error, Status.fromString(" "));
    try expectError(expected_error, Status.fromString("99"));
    try expectError(expected_error, Status.fromU16(99));
}
...
test "status code 101" {
    ...
    try expect(try Status.fromString("101") == Status.switching_protocols);
    try expect(try Status.fromU16(101) == Status.switching_protocols);
}
...
test "status code 200" {
    ...
    try expect(try Status.fromString("200") == Status.ok);
    try expect(try Status.fromU16(200) == Status.ok);
}
...
test "status code 201" {
    ...
    try expect(try Status.fromString("201") == Status.created);
    try expect(try Status.fromU16(201) == Status.created);
}
...
test "status code 202" {
    ...
    try expect(try Status.fromString("202") == Status.accepted);
    try expect(try Status.fromU16(202) == Status.accepted);
}
...
test "status code 203" {
    ...
    try expect(try Status.fromString("203") == Status.non_authoritative_information);
    try expect(try Status.fromU16(203) == Status.non_authoritative_information);
}
...
test "status code 204" {
    ...
    try expect(try Status.fromString("204") == Status.no_content);
    try expect(try Status.fromU16(204) == Status.no_content);
}
...
test "status code 205" {
    ...
    try expect(try Status.fromString("205") == Status.reset_content);
    try expect(try Status.fromU16(205) == Status.reset_content);
}
...
test "status code 206" {
    ...
    try expect(try Status.fromString("206") == Status.partial_content);
    try expect(try Status.fromU16(206) == Status.partial_content);
}
...
test "status code 300" {
    ...
    try expect(try Status.fromString("300") == Status.multiple_choices);
    try expect(try Status.fromU16(300) == Status.multiple_choices);
}
...
test "status code 301" {
    ...
    try expect(try Status.fromString("301") == Status.moved_permanently);
    try expect(try Status.fromU16(301) == Status.moved_permanently);
}
...
test "status code 302" {
    ...
    try expect(try Status.fromString("302") == Status.found);
    try expect(try Status.fromU16(302) == Status.found);
}
...
test "status code 303" {
    ...
    try expect(try Status.fromString("303") == Status.see_other);
    try expect(try Status.fromU16(303) == Status.see_other);
}
...
test "status code 304" {
    ...
    try expect(try Status.fromString("304") == Status.not_modified);
    try expect(try Status.fromU16(304) == Status.not_modified);
}
...
test "status code 307" {
    ...
    try expect(try Status.fromString("307") == Status.temporary_redirect);
    try expect(try Status.fromU16(307) == Status.temporary_redirect);
}
...
test "status code 308" {
    ...
    try expect(try Status.fromString("308") == Status.permanent_redirect);
    try expect(try Status.fromU16(308) == Status.permanent_redirect);
}
...
test "status code 400" {
    ...
    try expect(try Status.fromString("400") == Status.bad_request);
    try expect(try Status.fromU16(400) == Status.bad_request);
}
...
test "status code 401" {
    ...
    try expect(try Status.fromString("401") == Status.unauthorized);
    try expect(try Status.fromU16(401) == Status.unauthorized);
}
...
test "status code 403" {
    ...
    try expect(try Status.fromString("403") == Status.forbidden);
    try expect(try Status.fromU16(403) == Status.forbidden);
}
...
test "status code 404" {
    ...
    try expect(try Status.fromString("404") == Status.not_found);
    try expect(try Status.fromU16(404) == Status.not_found);
}
...
test "status code 405" {
    ...
    try expect(try Status.fromString("405") == Status.method_not_allowed);
    try expect(try Status.fromU16(405) == Status.method_not_allowed);
}
...
test "status code 406" {
    ...
    try expect(try Status.fromString("406") == Status.not_acceptable);
    try expect(try Status.fromU16(406) == Status.not_acceptable);
}
...
test "status code 407" {
    ...
    try expect(try Status.fromString("407") == Status.proxy_authentication_required);
    try expect(try Status.fromU16(407) == Status.proxy_authentication_required);
}
...
test "status code 408" {
    ...
    try expect(try Status.fromString("408") == Status.request_timeout);
    try expect(try Status.fromU16(408) == Status.request_timeout);
}
...
test "status code 409" {
    ...
    try expect(try Status.fromString("409") == Status.conflict);
    try expect(try Status.fromU16(409) == Status.conflict);
}
...
test "status code 410" {
    ...
    try expect(try Status.fromString("410") == Status.gone);
    try expect(try Status.fromU16(410) == Status.gone);
}
...
test "status code 411" {
    ...
    try expect(try Status.fromString("411") == Status.length_required);
    try expect(try Status.fromU16(411) == Status.length_required);
}
...
test "status code 412" {
    ...
    try expect(try Status.fromString("412") == Status.precondition_failed);
    try expect(try Status.fromU16(412) == Status.precondition_failed);
}
...
test "status code 413" {
    ...
    try expect(try Status.fromString("413") == Status.content_too_large);
    try expect(try Status.fromU16(413) == Status.content_too_large);
}
...
test "status code 414" {
    ...
    try expect(try Status.fromString("414") == Status.uri_too_long);
    try expect(try Status.fromU16(414) == Status.uri_too_long);
}
...
test "status code 415" {
    ...
    try expect(try Status.fromString("415") == Status.unsupported_media_type);
    try expect(try Status.fromU16(415) == Status.unsupported_media_type);
}
...
test "status code 416" {
    ...
    try expect(try Status.fromString("416") == Status.range_not_satisfiable);
    try expect(try Status.fromU16(416) == Status.range_not_satisfiable);
}
...
test "status code 417" {
    ...
    try expect(try Status.fromString("417") == Status.expectation_failed);
    try expect(try Status.fromU16(417) == Status.expectation_failed);
}
...
test "status code 421" {
    ...
    try expect(try Status.fromString("421") == Status.misdirected_request);
    try expect(try Status.fromU16(421) == Status.misdirected_request);
}
...
test "status code 426" {
    ...
    try expect(try Status.fromString("426") == Status.upgrade_required);
    try expect(try Status.fromU16(426) == Status.upgrade_required);
}
...
test "status code 500" {
    ...
    try expect(try Status.fromString("500") == Status.internal_server_error);
    try expect(try Status.fromU16(500) == Status.internal_server_error);
}
...
test "status code 501" {
    ...
    try expect(try Status.fromString("501") == Status.not_implemented);
    try expect(try Status.fromU16(501) == Status.not_implemented);
}
...
test "status code 502" {
    ...
    try expect(try Status.fromString("502") == Status.bad_gateway);
    try expect(try Status.fromU16(502) == Status.bad_gateway);
}
...
test "status code 503" {
    ...
    try expect(try Status.fromString("503") == Status.service_unavailable);
    try expect(try Status.fromU16(503) == Status.service_unavailable);
}
...
test "status code 504" {
    ...
    try expect(try Status.fromString("504") == Status.gateway_timeout);
    try expect(try Status.fromU16(504) == Status.gateway_timeout);
}
...
test "status code 505" {
    ...
    try expect(try Status.fromString("505") == Status.http_version_not_supported);
    try expect(try Status.fromU16(505) == Status.http_version_not_supported);
}
